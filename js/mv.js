window.onload=function(){
  ...
  var oBtn_getSong=window.parent.document.getElementById('btn_getSong');
	...
	function search(inpval,num,idom){
		ajax({
					type:'get',
					url:'https://api.imjad.cn/cloudmusic/?type=search&search_type=1004&s='+inpval+'&'+'limit'+'='+num,

					async:true,
					success:function(data){
		        //循环获取数据
		        for (var i = 0; i <data.result.mvs.length; i++) {
							//新建接受数据的li img p a标签；
							...
							//添加一个跳转到audio页面的a链接
							nA.setAttribute('href',"../content/video.html" );
							nA.setAttribute('target', 'my_frame');

					    //将获取图片数据绑定到img标签上；将获取的歌曲信息绑定到p上；
					    nImg.setAttribute('src',data.result.mvs[i].cover );
					    // alert(data.result.songs[i].al.picUrl);
					    nImg.setAttribute('data_id',data.result.mvs[i].id);
	            
	            nP.innerText=data.result.mvs[i].name;
	            nP.setAttribute('data_id',data.result.mvs[i].id);
	            nP.setAttribute('title',data.result.mvs[i].name);
	...
	search('张国荣','16',oSong_list);
	//获取最热歌曲的资源；
	search('经典','20',oHot_list);
